...
NUM_NEURONS_PER_LAYER    = [784,   128,  64,   10]            # needs tweaking!
#                           ^ input layer   
#                                  ^ first hidden layer
#                                        ^ second hidden layer
#                                              ^ output layer
...
def feedforward(input_sample):
    ...
    for i in range(0, NUM_WEIGHTS_MATRICES-1):
        neuron_list[i+1] = weight_matrices[i] @ neuron_list[i] + bias_list[i]

if __name__ == '__main__':
    # initialise (TODO: with random values) the NN structural components
    init_matrices()
    init_neuron_list()
    init_bias_list()
    ...
    # now take a sample and feed it forward through the NN, input_sample must be np.array()
    feedforward(np.zeros(784))